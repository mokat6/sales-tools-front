...
export const DataTableCursor = ({ data, onRowSelect, pagination }: DataTableCursorProps) => {
  ...
  const table = useReactTable<CompanyDto>({
    ...
    state: {
      rowSelection,
      pagination: {
        pageIndex: 0, // keep it fixed (just required by API, you don't actually paginate by index)
    ...
    onPaginationChange: (updater) => {
      const next = typeof updater === "function" ? updater({ pageIndex: 0, pageSize: pagination.pageSize }) : updater;
      console.log("onPaginationChange: (updater) => { IS RUNNING!!");
      pagination.setCursor(pagination.nextCursor);
    ...
    <div className="bg-bg-table text-text-body" onClick={() => console.log("log data ", rowSelection)}>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={() => pagination.setCursor(undefined)} // or null, depending on what your API expects for "first page"
          disabled={pagination.currentCursor === undefined} // or use a boolean like hasPreviousPage
        >
          {"<<"}
        </button>
        <button
        ...
        <button
          className="border rounded p-1"
          onClick={() => pagination.setCursor(pagination.nextCursor)}
