...
export class SwaggerSdk implements ISwaggerSdk {
  ...
  protected processCompany(response: Response): Promise<CompanyDto[]> {
    ...
    if (status === 200) {
      return response.text().then((_responseText) => {
        ...
        if (Array.isArray(resultData200)) {
          result200 = [] as any;
          for (let item of resultData200) result200!.push(CompanyDto.fromJS(item));
        } else {
          result200 = <any>null;
        }
        return result200;
  ...
  contact(compId?: number | undefined, signal?: AbortSignal): Promise<ContactDto[]> {
    ...
    if (compId === null) throw new Error("The parameter 'compId' cannot be null.");
    else if (compId !== undefined) url_ += "compId=" + encodeURIComponent("" + compId) + "&";
    url_ = url_.replace(/[?&]$/, "");
  ...
  protected processContact(response: Response): Promise<ContactDto[]> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && response.headers.forEach) {
      response.headers.forEach((v: any, k: any) => (_headers[k] = v));
    ...
    if (status === 200) {
      return response.text().then((_responseText) => {
        ...
        if (Array.isArray(resultData200)) {
          result200 = [] as any;
          for (let item of resultData200) result200!.push(ContactDto.fromJS(item));
        } else {
          result200 = <any>null;
        }
        return result200;
